...
/*
 * The two ways a rider leaves this screen without an error.
 *
 ...
 * large, the word is set in the display face at page size, and the completed
 * questionnaire is gone rather than greyed out behind it: a screen still full
 * of answers reads as "not finished yet" from that distance whatever it says at
 * the top.
 *
 * It is not a celebration. There is no confetti, no illustration and no second
 * call to action, because the next thing that happens is another rider, and the
 * screen's job is to get out of the way quickly.
 *
 * The wording says "on this device" deliberately. The tablet is routinely
 * offline and a message implying the response had reached a server would be
 * false for hours.
 *
 ...
 * status colours; this is information, so it is `info`.
 *
 * The guard is a question about a public code, so it exists on the sticker paths
 * only. A contact capture has no code to ask about, and this concept invents no
 * equivalent for it.
 */
